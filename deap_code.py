...
def evaluate(individual):
    x = obj_funct(individual,'DLSC_Temuco_multiobj',[797,798,800,801],[[4,11],[4,12]])
    return x
...
def main():
    ...
    for i in range(NGEN//NGENpar):
        ...
        for ind in range(0,len(hof)): 
            ...
            STS_size_hof.append(hof[ind][2])
            Solar_angle_hof.append(hof[ind][3])
            
            
        with open('DLSC_Temuco_multiobj_hof_'+str((i+1)*NGENpar)+'.csv','a') as writer:
            writer.writelines(map("{},{},{},{},{},{}\n".format, hof_fit_cost, hof_fit_emission, Solar_area_hof, LTS_size_hof, STS_size_hof, Solar_angle_hof))     
    return pop, hof
